...
namespace FXV
{
    ...
    public partial class Shield : fxvRenderObject
    {
        ...
        private Vector3 _LastShieldFadePoint = Vector3.up;
                ...
                if (activeHits.Count == 0)
                {
                    ...
                    if (sv && sv.camera)
                    {
                        if (properRenderers.Count == 0)
                        {
                            ...
                            if (properRenderers.Count == 0)
                            {
                                Debug.Log("could show hit preview as object is not ready " + gameObject.name, gameObject);
                                _DisableHitPreview();
                        ...
                        if (myColliders != null)
                        {
                            for (int i = 0; i < myColliders.Length; ++i)
                            {
                                bool wasEnabled = myColliders[i].enabled;
                                myColliders[i].enabled = true;
                                ...
                                myColliders[i].enabled = wasEnabled;
                            }
                        }
                        else
                        {
        ...
        public void SetMaterialVariant(int index, int materialIndex = -1)
        {
            ...
            {
                ShieldRenderChunk ch = (ShieldRenderChunk)renderChunks[i];
                if (materialIndex != -1 && materialIndex != ch.materialIndex)
                {
                    continue;
                }
...
#endif
                    if (j == 0) // update only once
                    {
                        ch.shieldMaterials = sm;
                        ch.SetNewMaterial(ch.renderers[j].sharedMaterials[ch.materialIndex]);
                    }
                }
            }
        }
        ...
        protected override void Update()
        {
         ...
         /*   if (Application.isPlaying)
            ...
            if (_LastShieldFadePoint != ShieldFadePoint)
            {
                SetShieldEffectFadePointPosition(ShieldFadePoint);
            }
        }
        ...
        {
            _LastShieldFadePoint = new Vector3(dir.x, dir.y, dir.z);
        ...
        private void AddHitMeshAtPos(Vector3 hitPos, Vector3 hitNormal, float hitScale, float hitDuration)
        {
            ...
            if (hit.StartHitFX(this, hitPos, tangent1, tangent2, hitColor, hitTextureColor, hitColorPower, hitDuration * hitDurationModifier, hitScale, hitDecalFadeAnimation, hitDecalSizeAnimation))
            {
                activeHits.Add(hit);
        ...
        {
            ShieldPostprocess.DestroyBuffers(hitCommandBuffers);
        }
        ...
        {
            if (!camera || (camera.cullingMask & renderLayersMask) == 0 || renderInstancingInfos.Count == 0)
            {
                return;
            }
        ...
        {
            if (!cam || (cam.cullingMask & renderLayersMask) == 0 || renderInstancingInfos.Count == 0)
            {
                return;
            }

...
namespace FXV
{
    ...
    public class fxvRenderObject : MonoBehaviour
    {
        internal protected class RenderChunk
        {
            ...
            public uint renderLayersMask = 0;

            public void AddRenderer(Renderer r)
            {
                renderers.Add(r);
                renderLayersMask |= r.renderingLayerMask;
            }
            ...
            public void SetNewMaterial(Material mat)
            {
                ...
                for (int k = 0; k < renderInstancingKey.Length; k++)
                {
                    renderInstancingKey[k] = parentRenderObject.GetInstancingKey(renderers[0], materialIndex, renderMesh, k);
                    parentRenderObject.OnInstancingKeyUpdate(k, renderInstancingKey[k]);
                }
            }
        ...
        protected uint renderLayersMask = 0;
        ...
        {
            ShieldPostprocess.OnInstancingKeyUpdate(this, keyIndex, renderKey);
        }

        internal protected virtual void OnAddToRenderList(RenderChunk rc)
        {
            ShieldPostprocess.AddToRenderList(rc);
        }
            ...
            for (int i = 0; i < properRenderers.Count; i++)
            {
                ...
                if (mesh || r is LineRenderer)
                {
                    ...
                    else
                    {
                        ...
                        rc.AddRenderer(r);
                        rc.renderMesh = mesh;
                        rc.subMeshIndex = mesh != null ? materialIdx % mesh.subMeshCount : 0;
                        ...
                        rc.isInstancingSupported = !forceDisableInstancing && rc.renderMaterial.enableInstancing && !(r is SkinnedMeshRenderer) && mesh != null;
                    ...
                    renderLayersMask |= rc.renderLayersMask;

//#define FXV_POSTPROCESS_DEBUG_LOGS
using UnityEngine;
...
namespace FXV
{
    ...
    static class PlayModeStateChanged
    {
        ...
        {
            ShieldPostprocess._ClearStatic();
            Shield._ClearStatic();
        }
    }
#endif
    ...
    public class ShieldPostprocess : MonoBehaviour
    {
        ...
        private static Dictionary<int, RenderInstanceInfo> renderInstancingInfos = new Dictionary<int, RenderInstanceInfo>();

        private static List<fxvRenderObject.RenderChunk> renderInstancedChunks = new List<fxvRenderObject.RenderChunk>();
        private static List<fxvRenderObject.RenderChunk> renderChunks = new List<fxvRenderObject.RenderChunk>();
        ...
        internal void _SetGaussianParams(Material mat)
        {
            ...
            float resolutionCompensation = Screen.height / 800.0f;
            ...
            mat.SetFloat("GAUSSIAN_TEXEL_SIZE_H", sampleStep * resolutionCompensation);
            mat.SetFloat("GAUSSIAN_TEXEL_SIZE_V", sampleStepV * resolutionCompensation);
        }
        ...
        internal static void AddToRenderList(fxvRenderObject ro)
        {
        ...
        internal static void AddToRenderList(fxvRenderObject.RenderChunk ch)
        {
        ...
        internal static void OnInstancingKeyUpdate(fxvRenderObject ro, int keyIndex, int renderKey)
        {
            if (!renderInstancingInfos.ContainsKey(renderKey))
            {
                RenderInstanceInfo rii = new RenderInstanceInfo();
...
#if FXV_POSTPROCESS_DEBUG_LOGS
                renderedMatrices+= rii.matricesCount;
                renderedInstances++;
#endif
                rii.matricesCount = 0; 
...
#if FXV_POSTPROCESS_DEBUG_LOGS
            Debug.Log("renderedInstances " + renderedInstances + " renderedInstancedMatrices " + renderedMatrices + " rendered nonInstanced " + renderedNonInstances);
#endif
        }
        ...
        internal static void _ClearStatic()
        {
            DestroyBuffers(shieldCommandBuffers);
            DestroyBuffers(grabCommandBuffers);
        }
        ...
        public void RenderWithCamera(Camera cam)
        {
            ...
            Shader.SetGlobalFloat("GLOBAL_ResolutionCompensation", Screen.height / 800.0f);
            ...
            bufferEvent.cmd.GetTemporaryRT(tempTargetTextureId[0], new RenderTextureDescriptor(rtW, rtH, RenderTextureFormat.DefaultHDR, 0));
            ...
            for (int i = 1; i < tempTargetTextureId.Length; i++)
            {
                ...
                bufferEvent.cmd.GetTemporaryRT(tempTargetTextureId[i], w, h, 0, FilterMode.Bilinear, RenderTextureFormat.DefaultHDR);
